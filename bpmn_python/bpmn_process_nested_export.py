...
class BpmnDiagramGraphNestedExport(object):
    ...
    def export_process_to_csv(bpmn_diagram, directory, filename):
        ...
        BpmnDiagramGraphNestedExport.export_node(bpmn_diagram, export_elements, start_node, nodes_classification)
        ...
        BpmnDiagramGraphNestedExport.write_export_node_to_file(file_object, export_elements)
        file_object.close()
    ...
    def export_node(bpmn_graph, export_elements, node, nodes_classification, order=0, prefix="", condition="", who="",
        ...
        if node_type == consts.Consts.start_event:
            return BpmnDiagramGraphNestedExport.export_start_event(bpmn_graph, export_elements, node, nodes_classification,
                                                                   order=order, prefix=prefix, condition=condition,
                                                                   who=who)
        elif node_type == consts.Consts.end_event:
            return BpmnDiagramGraphNestedExport.export_end_event(export_elements, node, order=order, prefix=prefix,
                                                                 condition=condition, who=who)
        else:
            return BpmnDiagramGraphNestedExport.export_element(bpmn_graph, export_elements, node, nodes_classification,
    ...
    def export_element(bpmn_graph, export_elements, node, nodes_classification, order=0, prefix="", condition="",
        ...
        if BpmnDiagramGraphNestedExport.classification_join in node_classification and not add_join:
        ...
        if BpmnDiagramGraphNestedExport.classification_split in node_classification:
            next_node = None
            ...
            for outgoing_flow_id in outgoing_flows:
                ...
                if BpmnDiagramGraphNestedExport.classification_join in nodes_classification[outgoing_node[0]]:
                    export_elements.append(
                        {"Order": next_prefix + str(1), "Activity": "goto " + prefix + str(order + 1),
                         "Condition": condition, "Who": who, "Subprocess": "", "Terminated": ""})
                elif outgoing_flow_id == default_flow_id:
                    tmp_next_node = BpmnDiagramGraphNestedExport.export_node(bpmn_graph, export_elements, outgoing_node,
            ...
            if next_node is not None:
                return BpmnDiagramGraphNestedExport.export_node(bpmn_graph, export_elements, next_node,
            ...
            outgoing_flow = bpmn_graph.get_flow_by_id(outgoing_flow_id)
            outgoing_node = bpmn_graph.get_node_by_id(outgoing_flow[2][consts.Consts.target_ref])
            return BpmnDiagramGraphNestedExport.export_node(bpmn_graph, export_elements, outgoing_node,
                                                            nodes_classification, order=(order + 1), prefix=prefix,
                                                            who=who)
        else:
            return None
    ...
    def export_start_event(bpmn_graph, export_elements, node, nodes_classification, order=0, prefix="", condition="",
        ...
        outgoing_flow = bpmn_graph.get_flow_by_id(outgoing_flow_id)
        outgoing_node = bpmn_graph.get_node_by_id(outgoing_flow[2][consts.Consts.target_ref])
        return BpmnDiagramGraphNestedExport.export_node(bpmn_graph, export_elements, outgoing_node, nodes_classification,
                                                        order + 1, prefix, who)
    ...
    def write_export_node_to_file(file_object, export_elements):
        ...
        def split(x):
            return list(map(lambda x: ''.join(x[1]), groupby(x, lambda x: x.isnumeric())))

        for elem in export_elements:
            elem['Order2'] = split(elem['Order'])

        def group_elements(elements):
            if not elements[0]['Order2']:
                for elem in elements:
                    del elem['Order2']
                return elements
            groups = [(x, list(y)) for x, y in groupby(elements, lambda x: x['Order2'][0])]
            for i in range(len(groups)):
                for j in range(len(groups[i][1])):
                    groups[i][1][j]['Order2'].pop(0)
                groups[i] = group_elements(groups[i][1])
            return groups
        ...
        def flatten(ls):
            if not isinstance(ls, list):
                ...
                return [flatten(x) for x in ls if x]

        grouped = group_elements(export_elements)
        flat = flatten(grouped)
        pprint(flat, stream=file_object)
